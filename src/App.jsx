...
const STOCKS = {
  samsung: {
    name: "삼성전자", fullName: "삼성전자 (005930.KS)", symbol: "005930.KS",
    currency: "KRW", base: 356000, vol: 0.025, trend: 0.001, purchase: null,
    fmt: v => v != null && !isNaN(v) ? Math.round(v).toLocaleString("ko-KR") + "원" : "-",
  },
  spacex: {
    name: "SpaceX (SPCX)", fullName: "SpaceX SPCX", symbol: "SPCX",
    currency: "USD", base: 196, vol: 0.024, trend: -0.001, purchase: 212,
...
const nf = (v, digits=2) => (v != null && !isNaN(v)) ? Number(v).toFixed(digits) : "0";
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    fetchYahoo(stock.symbol)
      .then(result=>{
        const cd=buildChart(result.data,stock);
        setChart(cd);setSigs(computeSignals(cd));
        setApiMeta({currentPrice:result.currentPrice,dayChange:result.dayChange,dayChangePct:result.dayChangePct});
        setDataStatus("real");
      })
      .catch(()=>{
  ...
  const runAnalysis=useCallback(async()=>{
    ...
    try{
      ...
      const prompt=`${stock.fullName} 분석. 현재가: ${stock.fmt(currentPrice)} | 전일비: ${pSign(safePct)}${nf(safePct)}%${stock.purchase?` | 매수가: ${stock.fmt(stock.purchase)} (${nf((currentPrice/stock.purchase-1)*100)}%)`:""}. RSI: ${l.rsi} | 매수신호: ${sigs.bullPct}%. 최신 뉴스, 거시 동향, 매수/매도/관망 의견.`;
  ...
  return(
    <div style={{background:"#0f1117",color:"#e0e6ed",fontFamily:"system-ui,-apple-system,sans-serif",display:"flex",flexDirection:"column",height:"100vh"}}>
      ...
      <div style={{background:"#13161f",padding:"8px 16px",borderBottom:"1px solid #2d3040",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",flexShrink:0,minHeight:48}}>
        {dataStatus==="loading"
          ?<span style={{color:"#7c8599",fontSize:13}}>📡 Yahoo Finance 수신 중…</span>
          :<>
        ...
        <div style={{overflowY:"auto",padding:"12px 10px 12px 14px",display:"flex",flexDirection:"column",gap:10}}>
          {dataStatus==="loading"&&(
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",flex:1,flexDirection:"column",gap:12,color:"#7c8599",paddingTop:80}}>
              <div style={{fontSize:36}}>📡</div><div style={{fontSize:14}}>Yahoo Finance 연결 중…</div>
            </div>
          )}
          {dataStatus!=="loading"&&(<>
            {/* Price chart */}
          ...
          <div style={{padding:14}}>
            ...
            {aiLoading&&(
              <div style={{textAlign:"center",padding:"22px 0",color:"#7c8599"}}>
                ...
                <div style={{fontSize:13}}>Gemini가 구글 검색 중…</div>
              </div>
            )}
            {analysis&&(
